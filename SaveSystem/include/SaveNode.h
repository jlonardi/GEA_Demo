...
public:
	...
	*/
	...
	void add(const std::string& id, TYPE* data, unsigned int length)
	{
		...
		SaveNode::dataToByteArray(data, sizeof(TYPE), length);
	...
	*/
	template <typename TYPE>
	void poll(const std::string& id, TYPE* data, unsigned int length)
	{
		// Checks that the reading range is valid
		...
		SaveNode::byteArrayToData(data, sizeof(TYPE), length, idIndex);
	}
